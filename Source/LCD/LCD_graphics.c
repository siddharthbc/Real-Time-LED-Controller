...
#define DRAW_RUNS_AS_RECTANGLES (1)
#define STEP 2
...
void Graphics_Test(void) {
	...
	c.G = 255;
	p2.X = 0;
	for (y=LCD_HEIGHT/2 + 1; y<LCD_HEIGHT; y += STEP) {
		p2.Y = y;
		LCD_Draw_Line(&p1, &p2, &c);
		LCD_Refresh();
	}
...
void Test_Draw_Line(void) {
	...
	PT_T p2[] = {
	// Horizontal
		{LCD_WIDTH/2+100, LCD_HEIGHT/2},
		{LCD_WIDTH/2-100, LCD_HEIGHT/2},
	// Vertical
		{LCD_WIDTH/2, LCD_HEIGHT/2+100},
		{LCD_WIDTH/2, LCD_HEIGHT/2-100},
	// Diagonal
		{LCD_WIDTH/2+100, LCD_HEIGHT/2+100},
		{LCD_WIDTH/2+100, LCD_HEIGHT/2-100},
		{LCD_WIDTH/2-100, LCD_HEIGHT/2+100},
		{LCD_WIDTH/2-100, LCD_HEIGHT/2-100},
		// X-major
		{LCD_WIDTH/2+100, LCD_HEIGHT/2+30},
		{LCD_WIDTH/2+100, LCD_HEIGHT/2-30},
		{LCD_WIDTH/2-100, LCD_HEIGHT/2+30},
		{LCD_WIDTH/2-100, LCD_HEIGHT/2-30},
	// Y-major
		{LCD_WIDTH/2+30, LCD_HEIGHT/2+100},
		{LCD_WIDTH/2+30, LCD_HEIGHT/2-100},
		{LCD_WIDTH/2-30, LCD_HEIGHT/2+100},
		{LCD_WIDTH/2-30, LCD_HEIGHT/2-100},
		{-1, -1}
	...
	LCD_Draw_Line(&p1, &p2[9], &green);
	while (1)
		;
	for (int i = 0; p2[i].X >= 0; i++)
		LCD_Draw_Line(&p1, &p2[i], &green);
