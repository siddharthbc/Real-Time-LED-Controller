...
void LCD_TS_Reconfigure_IO(int config) {
	switch (config) {
		case LCD_TS_CONFIG_DIG_TOUCH:
...
uint32_t LCD_TS_Read(PT_T * position) {
	...
	// Determine if screen is pressed.
	LCD_TS_Reconfigure_IO(LCD_TS_CONFIG_DIG_TOUCH); 
	// Wait for the inputs to settle
	osDelay(TS_DELAY);
	// Read XR via digital input. If >0, then screen is not pressed
	b = (LCD_TS_XR_PT->PDIR) & MASK(LCD_TS_XR_BIT);
	if (b>0) {		// Screen not pressed
		return 0;
	} 
	// Screen pressed, so continue
	// Read X Position
	LCD_TS_Reconfigure_IO(LCD_TS_CONFIG_ANA_X); 
	// Wait for inputs to settle
	osDelay(TS_DELAY);
		
#if USE_ADC_SERVER
	yd=request_conversion(LCD_TS_YD_ADC_CHAN, LCD_TS_YD_ADC_MUXSEL);
#else
	// Read inputs
	ADC0->SC1[0] = LCD_TS_YD_ADC_CHAN; // start conversion on channel YU
	...
	// Read Y Position
	LCD_TS_Reconfigure_IO(LCD_TS_CONFIG_ANA_Y); 
	// Wait for the inputs to settle
	osDelay(TS_DELAY);
	// Read inputs
#if USE_ADC_SERVER
	xr=request_conversion(LCD_TS_XR_ADC_CHAN, LCD_TS_XR_ADC_MUXSEL);
#else
	ADC0->SC1[0] = LCD_TS_XR_ADC_CHAN; // start conversion on channel XL
	while (!(ADC0->SC1[0] & ADC_SC1_COCO_MASK))
		;
	xr = ADC0->R[0];
#endif
